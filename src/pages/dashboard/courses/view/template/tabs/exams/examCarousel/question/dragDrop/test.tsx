...
export default function QuestionDragDrop({
  ...
  // const selectedOpt = thisQueAnswers?.selectedOpt
  //   ? JSON.parse(thisQueAnswers.selectedOpt)
  //   : null;
  // const [initialDraggables, setInitialDraggables] = useState([]);
  // const [initialDroppables, setInitialDroppables] = useState([]);
  // const [selectedOpt, setSelectedOpt2] = useState<{
  //   draggableItems: DraggableAreaProps[];
  //   droppableAreas: DroppableAreaProps[];
  // }>(
  ...
  useEffect(() => {
    setDraggableItems(
      thisQueAnswers?.selectedOpt
        ? JSON.parse(thisQueAnswers.selectedOpt).draggableItems
    ...
    setDroppableAreas(
      thisQueAnswers?.selectedOpt
        ? JSON.parse(thisQueAnswers.selectedOpt).droppableAreas
  ...
  //   selectedOpt?.draggableItems ||
  //   question?.question?.options.map(({ option: label }, id) => ({ id, label }));

  // const initialDroppables =
  ...
  const handleDropItem = (areaId: number, item: DraggableAreaProps) => {
    ...
    const answerstate = prevAreasArr.every((area) => area.items.length === 0)
      ? "skipped"
      : prevAreasArr.every(
